%%%
%  Ballbot nonlinear model derivation script
%
%   Contents:
...
%% Disturbance torques - human interaction with the body

syms Twx Twy;
% Twx influences only dthetax, and Twy only d_thetay
Jwx = [1 0 0 0 0]';
Jwy = [0 1 0 0 0]';

dist = Jwx*Twx + Jwy*Twy;
...
f_ = subs(f_, {T1, T2, T3}, {u1 u2 u3});
% disturbance
f_par = subs(f_, {Twx, Twy}, {w1 w2});
toc
...
w0 = [0 0]';
%% Linearisation
disp('Linearisation x0');
tic;
A_ = cell(1,10);
disp('A matrix');
fprintf('10/00\n');
for i = 1:10
    A_{i} = (subs(diff(n_model,x(i)), [x; u; w], [x0; u0; w0]));
    fprintf('\b\b\b%02d\n',i);
end
toc
B_ = cell(1,10);
...
tic
for i =1:3
    B_{i} = (subs(diff(n_model,u(i)), [x; u; w], [x0; u0; w0]));
end
toc
W_ = cell(1,2);
disp('W matrix');
tic
for i =1:2
    W_{i} =  (subs(diff(n_model,w(i)), [x; u; w], [x0; u0; w0]));
end
toc
